...
@implementation UIImage (ULKNinePatch)

static char ninePatchPaddingsKey;

+ (UIImage *)ulk_ninePatchImageFromImage:(UIImage *)image {
    UIEdgeInsets capInsets = UIEdgeInsetsZero;
    UIEdgeInsets padding = UIEdgeInsetsZero;
    BOOL hasPadding = FALSE;
    [image ulk_ninePatchCapInsets:&capInsets padding:&padding hasPadding:&hasPadding];
    UIImage *ninePatchImage = [self ulk_resizableImageFromNinePatchImage:image withInsets:capInsets];
    if (hasPadding) {
...
+ (BOOL)ulk_isNinePatchImageFile:(NSString *)fileName {
...
- (BOOL)ulk_hasNinePatchPaddings {
    return objc_getAssociatedObject(self, &ninePatchPaddingsKey) != nil;
}

- (UIEdgeInsets)ulk_ninePatchPaddings {
...
- (void)ulk_writePixelMaskForRect:(CGRect)rect toBuffer:(unsigned char *)buffer {
...
- (BOOL)ulk_startAndEndFromBuffer:(unsigned char *)buffer length:(long)length startRef:(CGFloat *)start endRef:(CGFloat *)end {
...
- (void)ulk_ninePatchCapInsets:(UIEdgeInsets *)capInsets padding:(UIEdgeInsets *)padding hasPadding:(BOOL *)hasPadding {
    // CapInsets rects
    ...
    if (totalBufferSize > 0) {
        ...
        if (capInsets != NULL) {
            [self ulk_writePixelMaskForRect:leftRect toBuffer:leftBuffer];
            [self ulk_writePixelMaskForRect:topRect toBuffer:topBuffer];
        }
        if (padding != NULL) {
            [self ulk_writePixelMaskForRect:rightRect toBuffer:rightBuffer];
            [self ulk_writePixelMaskForRect:bottomRect toBuffer:bottomBuffer];
        }
        
        // ...then find start and end pixel of the nine patch indicator lines
        if (capInsets != NULL) {
            [self ulk_startAndEndFromBuffer:leftBuffer length:verticalBufferSize startRef:&(*capInsets).top endRef:&(*capInsets).bottom];
            [self ulk_startAndEndFromBuffer:topBuffer length:horizontalBufferSize startRef:&(*capInsets).left endRef:&(*capInsets).right];
        ...
        if (padding != NULL) {
            BOOL paddingIndicator = [self ulk_startAndEndFromBuffer:rightBuffer length:verticalBufferSize startRef:&(*padding).top endRef:&(*padding).bottom];
            paddingIndicator |= [self ulk_startAndEndFromBuffer:bottomBuffer length:horizontalBufferSize startRef:&(*padding).left endRef:&(*padding).right];
...
+ (UIImage *)ulk_imageWithName:(NSString *)name fromBundle:(NSBundle *)bundle {
    ...
    if ([[UIScreen mainScreen] respondsToSelector:@selector(scale)] && [UIScreen mainScreen].scale >= 2.f) {
        ...
        if (retinaImageURL != nil) {
            ...
            if (retinaImage != nil) {
                if ([self ulk_isNinePatchImageFile:[retinaImageURL lastPathComponent]]) {
                    ret = [self ulk_ninePatchImageFromImage:retinaImage];
                } else {
                    ret = retinaImage;
                }
            }
    ...
    if (ret == nil) {
        ...
        if (imageURL != nil) {
            ...
            if ([self ulk_isNinePatchImageFile:[imageURL lastPathComponent]]) {
                ret = [self ulk_ninePatchImageFromImage:image];
            } else {
                ret = image;
            }
        } else {
...
+ (UIImage *)ulk_resizableImageFromNinePatchImage:(UIImage *)image withInsets:(UIEdgeInsets)insets {
